...
![Kernel Operator](/img/tut1/part3-kernel-operator.png){: .center-image }
...
Let's also remind oursevles what the scene looks like with no blurring applied.

![No blur](/img/tut1/part3-scene-noblur.png){: .center-image }

## Multi-pass

Let's take a look at a shader that implements a two-pass box blur first. I've provided a template shader as before - `Shaders/BoxBlurMultipass.shader`. There are a few things new things to go over.
...
A good time for a side note here: the way I've written this shader, the kernel will always end up with an odd number for N, even if you define `_KernelSize` as an even number. These two variables give us bounds for the loop we're about to write.
...
![Box Blur (Incorrect)](/img/tut1/part3-scene-boxblur-wrong.png){: .center-image }
...
![Box Blur](/img/tut1/part3-scene-boxblur.png){: .center-image }
...
A box blur does the job of blurring the image, but it performs terribly on object corners - there are other blurring algorithms that give finer results. A slightly more complicated algorithm is the Gaussian blur algorithm. It's similar to a box blur in that it entails convolving another square matrix with an image, but this time we'll calculate kernel values such that the central pixel is more heavily weighted than the surrounding pixels. The resulting blurred image will look much better on sharp corners than box blur does.
...
![Gaussian Blur](/img/tut1/part3-scene-gaussblur.png){: .center-image }
...
That's actually all the work that needs doing - the multi-pass shader did most of the heavy lifting for us. All being well, if you run this single-pass shader using the `ImageEffectBase` component, the result should be the same as the multi-pass version.
...
![Edge Blur](/img/tut1/part3-scene-edgeblur.png){: .center-image }
